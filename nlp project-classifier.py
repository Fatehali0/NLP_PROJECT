...
def get_wikipedia_content(title, lang='en'):
    user_agent = "NLP_Assignment1/1.0 (fatehmallah17@gmail.com)"  # Replace with your own user agent
...
def create_wikipedia_dataset(medical_title, non_medical_title, num_samples=500):
    ...
    data = pd.DataFrame({
        'text5': [medical_content] * num_samples + [non_medical_content] * num_samples,
...
# Data Preprocessing
dataset['text5'] = dataset['text5'].apply(lambda x: x.lower() if x else x)

# Feature Extraction using TF-IDF
vectorizer = TfidfVectorizer()
X = vectorizer.fit_transform(dataset['text5'])
y = dataset['label']
...
new_text = "This is a new text5 about medical topics."
new_text_vectorized = vectorizer.transform([new_text])
prediction = model.predict(new_text_vectorized)
print(f'Prediction for the new text5: {prediction}')
